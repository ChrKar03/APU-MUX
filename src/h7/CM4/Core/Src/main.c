...
//#define HRTIMB_PER_ADDR ((volatile uint32_t *)&HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].PERxR)
//#define HRTIMC_PER_ADDR ((volatile uint32_t *)&HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_C].PERxR)
//#define HRTIMD_PER_ADDR ((volatile uint32_t *)&HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_D].PERxR)
//#define HRTIME_PER_ADDR ((volatile uint32_t *)&HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_E].PERxR)

#define HRTIMA_CMP1 hhrtim.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP1xR
#define HRTIMB_CMP1 hhrtim.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].CMP1xR
#define HRTIMC_CMP1 hhrtim.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_C].CMP1xR
#define HRTIME_CMP1 hhrtim.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_E].CMP1xR
...
{
    if (htim->Instance == TIM1 || htim->Instance == TIM5)
    {
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
        	check[0] = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2);
        	HRTIMA_CMP1 = check[0];
        }
    }
    else if (htim->Instance == TIM2 || htim->Instance == TIM8)
    {
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
        	check[1] = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2);
        	HRTIMB_CMP1 = check[1];
        }
    }
    else if (htim->Instance == TIM3 || htim->Instance == TIM12)
	{
		if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
        	check[2] = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2);
        	HRTIMC_CMP1 = check[2];
		}
	}
    else if (htim->Instance == TIM4 || htim->Instance == TIM15)
	{
		if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
        	check[3] = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2);
        	HRTIME_CMP1 = check[3];
		}
	}
}
...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_HRTIM_WaveformOutputStart(&hhrtim, HRTIM_OUTPUT_TE1);
	HAL_HRTIM_WaveformCounterStart(&hhrtim, HRTIM_TIMERID_TIMER_E);
